...
export function AIAnalyticsPage({ onOpenSettings, settingsRevision }: AIAnalyticsPageProps) {
  ...
  return <div className="page-wrap ai-page">
    ...
    <nav className="ai-section-tabs" aria-label="Разделы AI Аналитики">
      ...
      <button className={activeSection === "reviews" ? "active" : ""} onClick={() => setActiveSection("reviews")}><MessageSquareText size={19} /><span><strong>Анализ отзывов пользователей</strong><small>Trustpilot, Capterra и новые источники через адаптеры</small></span></button>
    </nav>
