    ...
    <div className={`settings-drawer api-settings-drawer ${open ? "open" : ""}`} aria-hidden={!open}>
      ...
      {activeTab === "proxy" && <div className="api-settings-pane" role="tabpanel">
        <div className="setting-block review-proxy-block">
          <span className="setting-label">Прокси для отзывов и Reddit</span>
          <p>Chromium использует эту прокси при сборе пользовательских отзывов и в «Обзоре Reddit». Проверка выполняется в фоне и не оборвётся из-за таймаута веб-сервера.</p>
          {proxyLoading
