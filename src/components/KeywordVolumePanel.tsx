import { AlertTriangle, BarChart3, Check, CheckCircle2, ChevronDown, CircleGauge, Database, FileDown, Globe2, KeyRound, LoaderCircle, Plus, Search, Settings2, Table2, Upload, X } from "lucide-react";
import { useEffect, useMemo, useRef, useState } from "react";
import { ApiRequestError, collectKeywordVolumes } from "../api";
import { getKeywordProviderSettings, hasGoogleAdsKeywordSettings, hasKeywordsForFreeSettings } from "../keywordSettings";
...
import type { KeywordSurferImportRow, KeywordVolumeMetric, KeywordVolumeResponse, KeywordVolumeSource } from "../shared/types";
...
const sourceInfo: Record<KeywordVolumeSource, { label: string; caption: string; icon: typeof BarChart3 }> = {
  ...
  keyword_surfer: { label: "Keyword Surfer", caption: "Импорт CSV из бесплатного расширения", icon: CircleGauge },
  keywords_for_free: { label: "Keywords For Free", caption: "Приблизительная оценка · 10 запросов бесплатно", icon: Database },
};
...
export function KeywordVolumePanel({ onOpenSettings, settingsRevision }: KeywordVolumePanelProps) {
  ...
  const [sources, setSources] = useState<KeywordVolumeSource[]>(["google_ads", "keyword_surfer", "keywords_for_free"]);
  ...
  const freeConfigured = useMemo(() => hasKeywordsForFreeSettings(), [settingsRevision]);
    ...
    try {
      setResult(await collectKeywordVolumes({
        ...
        credentials: {
          ...
          ...(sources.includes("keywords_for_free") && freeConfigured ? { keywordsForFreeApiKey: settings.keywordsForFreeApiKey } : {}),
        },
  ...
  return <div className="keyword-volume-panel">
    <section className="keyword-volume-builder">
      ...
      <section className="keyword-source-block"><header><span>3. Источники данных</span><button type="button" onClick={onOpenSettings}><Settings2 size={15} />Настроить API</button></header><div>{(Object.keys(sourceInfo) as KeywordVolumeSource[]).map((source) => {
        ...
        const configured = source === "google_ads" ? googleConfigured : source === "keywords_for_free" ? freeConfigured : surferRows.length > 0;
        return <button type="button" key={source} className={sources.includes(source) ? "selected" : ""} onClick={() => toggleSource(source)}><i>{sources.includes(source) && <Check size={14} />}</i><span><b><InfoIcon size={17} />{sourceInfo[source].label}</b><small>{sourceInfo[source].caption}</small></span><em className={configured ? "ready" : "missing"}>{configured ? source === "keyword_surfer" ? `${new Set(surferRows.map((row) => row.country)).size} стран` : "настроен" : source === "keyword_surfer" ? "нужен CSV" : "нужен ключ"}</em></button>;
      })}</div></section>

      {sources.includes("keyword_surfer") && <section className="surfer-import-block"><div><span><Upload size={18} /></span><div><strong>Импорт Keyword Surfer</strong><p>В расширении выберите страну, добавьте ключи в коллекцию и экспортируйте CSV. Здесь укажите ту же страну.</p></div></div><div className="surfer-import-controls"><select value={importCountry} onChange={(event) => setImportCountry(event.target.value)}><option value="">Страна</option>{countries.map((country) => <option key={country} value={country}>{META_COUNTRIES.find((option) => option.value === country)?.label ?? country} · {country}</option>)}</select><label className={!importCountry ? "disabled" : ""}><Upload size={15} />Выбрать CSV<input type="file" accept=".csv,text/csv" disabled={!importCountry} onChange={(event) => { void importSurfer(event.target.files?.[0]); event.currentTarget.value = ""; }} /></label></div>{importMessage && <p className="surfer-import-message">{importMessage}</p>}{surferRows.length > 0 && <div className="surfer-imported-countries">{[...new Set(surferRows.map((row) => row.country))].map((country) => <span key={country}><CheckCircle2 size={13} />{country} · {surferRows.filter((row) => row.country === country).length}<button type="button" onClick={() => setSurferRows((current) => current.filter((row) => row.country !== country))}><X size={12} /></button></span>)}</div>}</section>}
